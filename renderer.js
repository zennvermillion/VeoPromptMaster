document.addEventListener('DOMContentLoaded', () => {
    ...
    function createQueueItem(file) { const { name, path } = file; if (!productionQueueEl) return; const emptyQueueEl = productionQueueEl.querySelector('.empty-queue'); if (emptyQueueEl) emptyQueueEl.remove(); const uniqueId = `queue-item-${name.replace(/[^a-zA-Z0-9]/g, '')}`; if (document.getElementById(uniqueId)) return; const itemDiv = document.createElement('div'); itemDiv.className = 'queue-item'; itemDiv.id = uniqueId; itemDiv.innerHTML = `<div class="queue-item-icon">${ICONS.video}</div><div class="queue-item-info"><div class="filename" title="${name}">${name}</div><div class="status">Menunggu diproses...</div></div><div class="queue-item-action"><button class="primary generate-meta-queue-btn">Generate Metadata</button></div>`; productionQueueEl.appendChild(itemDiv); const processBtn = itemDiv.querySelector('.generate-meta-queue-btn'); processBtn.addEventListener('click', async () => { if (!activePrompt) { showNotification("Error: Tidak ada 'Prompt Aktif'. Buat atau pilih prompt di Tab Generator."); return; } processBtn.disabled = true; processBtn.textContent = "Memproses..."; const statusEl = itemDiv.querySelector('.status'); if(statusEl) statusEl.textContent = 'Menghubungi AI...'; const result = await window.api.generateMetadataAndCsv({ videoFilename: name, videoFilepath: path, activePrompt: activePrompt }); if (result.error) { if(statusEl) statusEl.textContent = `Gagal: ${result.error}`; processBtn.textContent = "Coba Lagi"; processBtn.disabled = false; } else { if(statusEl) statusEl.textContent = `Selesai! Metadata ditambahkan ke batch.`; processBtn.innerHTML = `${ICONS.processed} Selesai`; processBtn.classList.add('processed'); const metadataItem = { Filename: name, Title: result.title, Keywords: result.keywords, 'Generated by AI': 'YES' }; metadataBatch.push(metadataItem); localStorage.setItem("veoMetadataBatch_v2", JSON.stringify(metadataBatch)); updateExportUI(); } }); }
